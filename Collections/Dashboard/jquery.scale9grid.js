...
(function ($) {
	...
	} else if ($.browser.mozilla && $.browser.version.substr(0, 3) == "1.9" && parseFloat($.browser.version.substr(3)) > 1.0) {
		supportsBorderImage = true;
		...
		scale9Grid: function (settings) {
			...
			return $(this).each(function () {
				...
				var layoutGrid = function () {
					...
					if (width < gridLeft + gridRight || height < gridTop + gridBottom || (width == lastWidth && height == lastHeight)) {
						return;
					}
					if (supportsBorderImage) {
						$background.css({

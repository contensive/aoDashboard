...
set collectionName=Design Blocks

rem -- name of the collection folder, (should NOT include ao prefix)
set collectionPath=..\collections\Design Blocks\

rem -- name of the solution. SHOULD include ao prefix
set solutionName=aoDesignBlocks.sln

rem -- name of the solution. SHOULD include ao prefix
set binPath=..\server\aoDesignBlocks\bin\%DebugRelease%\net472\

rem -- name of the solution. SHOULD include ao prefix
set msbuildLocation=C:\Program Files (x86)\Microsoft Visual Studio\2019\Community\MSBuild\Current\Bin\

rem -- name of the solution. SHOULD include ao prefix
set deploymentFolderRoot=C:\Deployments\aoDesignBlocks\Dev\
...
rem
rd /S /Q "..\server\aoDesignBlocks\bin"
rd /S /Q "..\server\aoDesignBlocks\obj"
...
copy "..\ui\Tile\*.html" "%collectionpath%\"
copy "..\ui\Tile\*.css" "%collectionpath%\"

rem ==============================================================
rem
echo build aoDesignBlocks 
rem
...
dotnet build DesignBlockBase/DesignBlockBase.csproj --configuration %DebugRelease% --no-dependencies /property:Version=%versionNumber% /property:AssemblyVersion=%versionNumber% /property:FileVersion=%versionNumber%
if errorlevel 1 (
...
dotnet build aoDesignBlocks/aoDesignBlocks.csproj --configuration %DebugRelease% --no-dependencies /property:Version=%versionNumber% /property:AssemblyVersion=%versionNumber% /property:FileVersion=%versionNumber%
if errorlevel 1 (
   echo failure building aoDesignBlocks
   pause
   exit /b %errorlevel%
)

dotnet pack DesignBlockBase/DesignBlockBase.csproj --configuration %DebugRelease% --no-build --no-restore /property:PackageVersion=%versionNumber%

xcopy ".\DesignBlockBase\bin\%DebugRelease%\Contensive.DesignBlockBase.%versionNumber%.nupkg" "C:\nugetLocalPackages" /Y
xcopy ".\DesignBlockBase\bin\%DebugRelease%\Contensive.DesignBlockBase.%versionNumber%.nupkg" "%deploymentFolderRoot%%versionNumber%" /Y

...
Namespace Contensive.Addons.aoDashboard.Models
    ...
    Public Class wrapperModel
        ...
        Public ReadOnly Property foreignModelList(cp As CPBaseClass) As List(Of dummyForeignModelClass) '<-- NOTE - set this to the model of the records included in this list, NOT wrapperModel
            Get
                If (_foreignModelList Is Nothing) Then
                    _foreignModelList = dummyForeignModelClass.getObjectList(cp, id) '<-- NOTE - set this to the model of the records included in this list, NOT wrapperModel
                End If
                Return _foreignModelList
            End Get
        End Property
        Private _foreignModelList As List(Of dummyForeignModelClass) = Nothing '<-- NOTE - set this to the model of the records included in this list, NOT wrapperModel
        '
        ...
        Public Shared Function create(cp As CPBaseClass, recordId As Integer) As wrapperModel
            Dim result As wrapperModel = Nothing
            Try
                If recordId <> 0 Then
        ...
        Public Shared Function create(cp As CPBaseClass, recordGuid As String) As wrapperModel
            Dim result As wrapperModel = Nothing
            Try
        ...
        Private Shared Function loadObject2(cp As CPBaseClass, sqlCriteria As String) As wrapperModel
            Dim result As wrapperModel = Nothing
            Try
                Dim cs As CPCSBaseClass = cp.CSNew()
                If cs.Open(cnPrimaryContent, sqlCriteria) Then
                    result = New wrapperModel
                    With result
                        .id = cs.GetInteger("id")
        ...
        ''' <param name="someCriteria"></param>
        ''' <returns></returns>
        Public Shared Function getObjectList(cp As CPBaseClass, someCriteria As Integer) As List(Of wrapperModel)
            Dim result As New List(Of wrapperModel)
            Try
                Dim cs As CPCSBaseClass = cp.CSNew()
                If (cs.Open(cnPrimaryContent, "(someCriteria=" & someCriteria & ")", "name", True, "id")) Then
                    Dim instance As wrapperModel
                    Do
                        instance = wrapperModel.create(cp, cs.GetInteger("id"))
